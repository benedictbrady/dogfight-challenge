...
def train(
    ...
    # Periodic volume commit so dashboard can see live data
    commit_stop = threading.Event()
    def _periodic_commit():
        while not commit_stop.wait(60):  # every 60 seconds
            try:
                vol.commit()
            except Exception as e:
                print(f"[vol.commit] {e}")
...
def train_unified_modal(config: dict) -> dict:
    ...
    exp_name = config.get("_name") or _make_name(tag, ts)
    ...
    # Select training script based on config
    script_name = config.get("script", "train_unified")
    script_map = {
        "train_unified": "train_unified.py",
        "train_unified_dr": "train_unified_dr.py",
    }
    script_file = script_map.get(script_name, "train_unified.py")

    cmd = [
        "python", "-u", f"/app/training/{script_file}",
        "--config", config_path,
        "--checkpoint-dir", ckpt_dir,
        "--log-dir", log_dir,
        "--pool-dir", pool_dir,
...
def main(
    ...
    if config:
        ...
        if script in ("train_unified", "train_unified_dr"):
            print(f"Launching {script} pipeline from config: {config}")
