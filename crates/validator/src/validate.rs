...
mod tests {
    ...
    // --- Parameter limit boundary tests ---
    // Generated by: python3 crates/validator/tests/gen_param_limit_models.py
    // param_limit_exact.onnx: exactly 250,000 params (the maximum allowed)
    // param_limit_over.onnx:  exactly 250,001 params (4 bytes over the limit)
    ...
    #[test]
    fn test_param_limit_exact_passes() {
        let path = fixtures_dir().join("param_limit_exact.onnx");
        if !path.exists() {
            eprintln!(
                "Skipping test: {} not found (run gen_param_limit_models.py first)",
                path.display()
            );
            return;
        }
        ...
        let bytes = std::fs::read(&path).unwrap();
        let (count, _ops) = count_all_parameters(&bytes).unwrap();
        assert_eq!(count, MAX_PARAMETERS, "should have exactly 250,000 params");

        let report = validate_model_file(&path).unwrap();
        assert_eq!(report.parameter_count, MAX_PARAMETERS);
    }

    #[test]
    fn test_param_limit_over_fails() {
        let path = fixtures_dir().join("param_limit_over.onnx");
        if !path.exists() {
            eprintln!(
                "Skipping test: {} not found (run gen_param_limit_models.py first)",
                path.display()
        ...
        let bytes = std::fs::read(&path).unwrap();
        let (count, _ops) = count_all_parameters(&bytes).unwrap();
        assert_eq!(count, MAX_PARAMETERS + 1, "should have exactly 250,001 params");

        let result = validate_model_file(&path);
        assert!(result.is_err(), "validation should reject 250,001 params");
        match result.unwrap_err() {
            ValidationError::TooManyParameters(got, max) => {
                assert_eq!(got, MAX_PARAMETERS + 1);
                assert_eq!(max, MAX_PARAMETERS);
            }
