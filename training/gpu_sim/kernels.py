...
W_CONFIG_NORM_MAX_TURN_RATE = wp.constant(CONFIG_NORM_MAX_TURN_RATE)
...
def compute_observation(
    # Output observation array [N, OBS_SIZE]
    ...
    """Write 46-float observation for one player."""
    me = env_id * 2 + player
    ...
    # --- Self state (6 floats) ---
    obs[env_id, 0] = speed[me] / max_spd
    obs[env_id, 1] = wp.cos(yaw[me])
    obs[env_id, 2] = wp.sin(yaw[me])
    obs[env_id, 3] = float(hp[me]) / max_hp_val
    if gun_cd > 0.0:
        obs[env_id, 4] = float(cooldown[me]) / gun_cd
    else:
        obs[env_id, 4] = 0.0
    obs[env_id, 5] = pos_y[me] / W_MAX_ALTITUDE
    ...
    obs[env_id, 6] = rel_x / W_ARENA_DIAMETER
    obs[env_id, 7] = rel_y / W_ARENA_DIAMETER
    obs[env_id, 8] = speed[opp] / max_spd
    obs[env_id, 9] = wp.cos(yaw[opp])
    obs[env_id, 10] = wp.sin(yaw[opp])
    obs[env_id, 11] = float(hp[opp]) / max_hp_val
    obs[env_id, 12] = dist / W_ARENA_DIAMETER

    # --- Bullets (8 slots x 4 floats = 32 floats) ---
    # Find 8 nearest bullets via 8 passes, each finding the closest unused one.
    # Use 8 separate tracked indices to mark "used" bullets.
    # Track which slots are selected (up to 8)
    sel0 = int(-1)
    sel1 = int(-1)
    sel2 = int(-1)
    sel3 = int(-1)
    ...
    # Write bullet observation slots
    # Helper: write one bullet slot into obs
    # Slot 0
    base_idx = 13
    if sel0 >= 0:
        slot = bul_base + sel0
        ...
        obs[env_id, base_idx + 0] = brx / W_ARENA_DIAMETER
        obs[env_id, base_idx + 1] = bry / W_ARENA_DIAMETER
        if bul_owner[slot] == player:
            obs[env_id, base_idx + 2] = 1.0
        else:
            obs[env_id, base_idx + 2] = 0.0
        angle = wp.atan2(bry, brx)
        obs[env_id, base_idx + 3] = angle / PI
    else:
        obs[env_id, base_idx + 0] = 0.0
        obs[env_id, base_idx + 1] = 0.0
        obs[env_id, base_idx + 2] = 0.0
        obs[env_id, base_idx + 3] = 0.0
    ...
    if ticks_remaining < 0:
        ticks_remaining = 0
    obs[env_id, 45] = float(ticks_remaining) / float(W_MAX_TICKS)
...
def compute_config_obs(
    ...
    config_obs[env_id, 10] = cfg_max_turn_rate[env_id] / W_CONFIG_NORM_MAX_TURN_RATE
...
def step_with_action_repeat(
    ...
    # Compute observations for both players
    final_tick = tick_counter[env_id]

    compute_observation(
        obs_p0, pos_x, pos_y, yaw, speed, hp, cooldown, alive,
        bul_pos_x, bul_pos_y, bul_vel_x, bul_vel_y, bul_owner, bul_ticks,
        cfg_max_speed, cfg_max_hp, cfg_gun_cooldown,
        env_id, 0, final_tick,
    )
    compute_observation(
        obs_p1, pos_x, pos_y, yaw, speed, hp, cooldown, alive,
        bul_pos_x, bul_pos_y, bul_vel_x, bul_vel_y, bul_owner, bul_ticks,
        cfg_max_speed, cfg_max_hp, cfg_gun_cooldown,
        env_id, 1, final_tick,
    )
