...
def export_onnx_checkpoint(model, ckpt_dir, name, hidden=256, n_blocks=0):
    """Export actor-only ONNX alongside a .pt checkpoint.

    Args:
        model: ActorCritic model (or torch.compile wrapper)
        ckpt_dir: Directory containing checkpoints
        name: Checkpoint name (e.g., "step_100000", "final")
        hidden: Hidden dimension of the model
        n_blocks: Number of residual blocks
    """
    try:
        from model import ActorOnly, OBS_SIZE, ACTION_SIZE

        raw_model = getattr(model, "_orig_mod", model)
        actor = ActorOnly.from_actor_critic(raw_model)
        actor.eval()

        dummy = torch.randn(1, OBS_SIZE)
        onnx_path = str(Path(ckpt_dir) / f"{name}.onnx")
        ...
        print(f"  [onnx] WARNING: export failed for {name}: {e}")
