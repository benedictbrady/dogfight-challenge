...
def get_dr_ranges(update: int, dr_narrow_until: int) -> dict:
    """Return DR ranges based on schedule. Narrow early, full after threshold."""
...
def ppo_update(model, optimizer, obs_buf, act_buf, logp_buf, rew_buf, done_buf,
               val_buf, next_obs_t, next_done_t, args, device, scaler=None):
...
def train_unified_dr(args):
    ...
    print(f"Domain randomization: narrow until update {args.dr_narrow_until}, then full")
    ...
    # Set initial DR ranges (narrow)
    dr_ranges = get_dr_ranges(0, args.dr_narrow_until)
    vec_env.set_domain_randomization(dr_ranges)
    current_dr_mode = "narrow"
    ...
    for update in range(args.curriculum_updates):
        ...
        # DR schedule: widen from narrow to full
        new_dr_ranges = get_dr_ranges(total_updates, args.dr_narrow_until)
        new_dr_mode = "narrow" if total_updates < args.dr_narrow_until else "full"
        if new_dr_mode != current_dr_mode:
            vec_env.set_domain_randomization(new_dr_ranges)
            current_dr_mode = new_dr_mode
            print(f"\n>>> DR widened to full at update {total_updates}")
        ...
        metrics = ppo_update(model, optimizer, obs_buf, act_buf, logp_buf, rew_buf,
                             done_buf, val_buf, next_obs_t, next_done_t, args, device, scaler)
        ...
        writer.add_scalar("dr/mode", 0 if current_dr_mode == "narrow" else 1, global_step)
    ...
    # Ensure full DR is active
    vec_env.set_domain_randomization(DR_FULL)
    current_dr_mode = "full"
